 ...
 */
if (window['dojo'])
	dojo.provide('dwr.util');
if (typeof window['dwr'] == 'undefined')
	window.dwr = {};
if (typeof dwr['util'] == 'undefined')
	dwr.util = {};
...
dwr.util._setValuesRecursive = function(data, idpath, depth, options) {
	...
	if (dwr.util._isArray(data) && data.length > 0
		...
		for (var i = 0; i < data.length; i++) {
			dwr.util._setValuesRecursive(data[i], idpath + "[" + i + "]", depth
							- 1, options);
		}
	}
...
dwr.util.getValues = function(data, options) {
	if (typeof data == "string" || dwr.util._isHTMLElement(data)) {
		...
		if (options != null && options.prefix)
			prefix = options.prefix;
		if (options != null && options.idPrefix)
			prefix = options.idPrefix;
...
dwr.util.getFormValues = function(eleOrNameOrId) {
	...
	if (typeof eleOrNameOrId == "string") {
		...
		if (ele == null) {
			ele = dwr.util.byId(eleOrNameOrId); // arg is id
		}
	...
	if (ele != null) {
		...
		for (var i = 0; i < ele.elements.length; i++) {
			if (ele[i].type in {
				...
				continue;
			if (ele[i].name) {
				name = ele[i].name;   //史文峰进行修改
				//value = dwr.util.getValue(name);
				value = $('#'+eleOrNameOrId).find('[name="'+name+'"]').val();
			} else {
				if (ele[i].id)
					name = ele[i].id;
				else
...
dwr.util._getValuesRecursive = function(data, idpath, depth, options) {
	...
	if (dwr.util._isArray(data) && data.length > 0
			&& dwr.util._isObject(data[0])) {
		for (var i = 0; i < data.length; i++) {
			dwr.util._getValuesRecursive(data[i], idpath + "[" + i + "]", depth
							- 1, options);
		}
	}
	// Object (not array) -> handle nested object properties
...
dwr.util._isArrayLike = function(data) {
	return data
			...
			&& ((data.propertyIsEnumerable && data
					...
					|| !data.constructor || data != "[object Object]") // .length must be native prop
...
dwr.util._isFunction = function(data) {
	return (data
			&& (typeof data == "function" || Object.prototype.toString
					.call(data) == "[object Function]") && data != "[object NodeList]"); // need to workaround NodeList on Safari
};
